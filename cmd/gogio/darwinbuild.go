...
const minDarwinVersion = "9.0"

func buildDarwin(tmpDir, target string, bi *buildInfo) error {
	appName := bi.name
	switch *buildMode {
	case "archive":
		framework := *destPath
		...
		return archiveDarwin(tmpDir, target, framework, bi)
	case "exe":
		out := *destPath
		if out == "" {
			out = appName + ".ipa"
		...
		if err := archiveDarwin(tmpDir, target, tmpFramework, bi); err != nil {
			return err
		}
		...
		if !forDevice {
			return exeDarwin(tmpDir, target, out, bi)
		}
		...
		if err := exeDarwin(tmpDir, target, appDir, bi); err != nil {
			return err
		}
		if err := signDarwin(bi, tmpDir, appDir); err != nil {
			return err
		}
		return zipDarwinDir(out, tmpDir, "Payload")
	default:
		panic("unreachable")
	}
}

func signDarwin(bi *buildInfo, tmpDir, app string) error {
...
func exeDarwin(tmpDir, target, app string, bi *buildInfo) error {
	if bi.appID == "" {
...
int main(int argc, char * argv[]) {
	...
	for _, a := range bi.archs {
		clang, cflags, err := darwinCompilerFor(target, a)
		if err != nil {
			return err
		}
	...
	infoPlist := buildDarwinInfoPlist(bi)
	...
	if _, err := os.Stat(bi.iconPath); err == nil {
		assetPlist, err := darwinIcons(bi, tmpDir, app, bi.iconPath)
		if err != nil {
			return err
		}
...
func darwinIcons(bi *buildInfo, tmpDir, appDir, icon string) (string, error) {
	...
	compile := exec.Command(
		"actool",
		"--compile", appDir,
		"--platform", darwinPlatformFor(bi.target),
		"--minimum-deployment-target", minDarwinVersion,
		"--app-icon", "AppIcon",
...
func buildDarwinInfoPlist(bi *buildInfo) string {
	appName := strings.Title(bi.name)
	platform := darwinPlatformFor(bi.target)
...
</plist>`, appName, bi.appID, appName, bi.version, bi.version, platform, minDarwinVersion, supportPlatform, platform)
}

func darwinPlatformFor(target string) string {
	switch target {
	case "ios":
		return "iphoneos"
	case "tvos":
...
func archiveDarwin(tmpDir, target, frameworkRoot string, bi *buildInfo) error {
	...
	tags := bi.tags
	goos := "ios"
	supportsIOS, err := supportsDarwinGOOS("ios")
	if err != nil {
		return err
	}
	if !supportsIOS {
...
func supportsDarwinGOOS(wantGoos string) (bool, error) {
...
func darwinCompilerFor(target, arch string) (string, []string, error) {
	...
	cflags := []string{
		...
		"-isysroot", sdkPath,
		"-m" + platformOS + "-version-min=" + minDarwinVersion,
	}
	return clang, cflags, nil
}

func zipDarwinDir(dst, base, dir string) (err error) {
	f, err := os.Create(dst)
	if err != nil {
		return err
	}
